...
/// Стиль интерфейса (§7, HUD): данные, а не код — правится в редакторе и
/// едет в блит-шейдер юниформом. Сердце и блик — битмаски 9×9 (старший из
/// 9 бит = левый столбец), всё остальное — цвета.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct HudStyle {
    /// Силуэт сердца, 9 строк по 9 бит.
    pub heart: [u16; 9],
    /// Блик поверх заливки, те же координаты.
    pub glint: [u16; 9],
    pub full: [u8; 3],      // полное сердце
    pub empty: [u8; 3],     // потерянное
    pub outline: [u8; 3],   // обводка
    pub highlight: [u8; 3], // блик
    pub bar_bg: [u8; 4],    // подложка хотбара (rgb + сила наложения)
    pub bar_border: [u8; 3],
    pub bar_sel: [u8; 3],   // рамка выбранного слота
}

/// Дефолтный HUD «в духе беты» — стартовая точка для редактора.
pub const HUD: HudStyle = HudStyle {
    heart: [0x0D8, 0x1FC, 0x1FC, 0x1FC, 0x0F8, 0x070, 0x020, 0, 0],
    glint: [0, 0x180, 0x180, 0, 0, 0, 0, 0, 0],
    full: [209, 31, 36],
    empty: [41, 10, 13],
    outline: [0, 0, 0],
    highlight: [255, 168, 168],
    bar_bg: [13, 13, 13, 199],
    bar_border: [56, 56, 56],
    bar_sel: [235, 235, 235],
};

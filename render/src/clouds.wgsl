...
/// Клетка облачного растра, блоков (крупные «пиксели» слоя).
const CELL: f32 = 6.0;
...
fn fs_main(in: VsOut) -> @location(0) vec4<f32> {
    ...
    // Два масштаба: крупные поля задают кучность, мелкие клетки — рваный
    // блочный контур, как у классических облаков-плит.
    let field = hash2(vec2<i32>(floor(p / (CELL * 8.0))));
    let cell = hash2(vec2<i32>(floor(p / CELL)));
    if field > 0.55 || cell > 0.62 {
        discard;
    }
